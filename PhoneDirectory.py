...
def phone(strng, num):
    if len(re.findall(num, strng)) > 1:
        return f"Error => Too many people: {num}"
    elif len(re.findall(num, strng)) == 0:
        return f"Error => Not found: {num}"
    for line in strng.split('\n'):
        if num in line:
            line =  line.replace(";", "").replace("?", "").replace("+", "").replace("/", "").replace("!", "").replace(",", "").replace("_", " ").replace("$", "").replace("*", "").replace(":", "").strip(" ").split(" ")
            name = []
            for ele in line:
                if "<" in ele or ">" in ele:
                    name.append(ele)
                elif "-" in ele.replace("-", "", 1):
                    number = ele
            address = " ".join(line).replace(" ".join(name), "").replace(number, "").split(" ")
            address = " ".join([a for a in address if a != ""])
            name = " ".join(name).replace("<", "").replace(">", "")
            result = f"Phone => {number}, Name => {name}, Address => {address}"
            return result
